...
function __init__()
    ...
    if is_linux()
        if !(vnum.major == 1 && vnum.minor == 2)
            depsdir = realpath(joinpath(dirname(@__FILE__),"..","deps"))
            error("Current SCS version installed is $(SCS_version()), but we require version 1.2.*. On Linux and Windows, delete the contents of the `$depsdir` directory except for the files `build.jl` and `.gitignore`, then rerun Pkg.build(\"SCS\"). On OS X, run `using Homebrew; Homebrew.update()` in Julia.")
        end
    elseif !(vnum.major == 1 && vnum.minor == 1)
        depsdir = realpath(joinpath(dirname(@__FILE__),"..","deps"))
